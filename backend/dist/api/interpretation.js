 ...
 * Define enum values
 */
function defineEnumValue(field, value) {
    const definitions = {
        status: {
 ...
 */
function generateInterpretation(query, inference) {
    const lowerQuery = query.toLowerCase();
    ...
    if (lowerQuery.includes('category') || lowerQuery.includes('categories')) {
        if (inference.categories && inference.categories.length > 0) {
            const categoryInfo = inference.categories.map((cat, idx) => `Category ${idx + 1}: The system recorded category '${cat.category}' with message "${cat.message}".`).join(' ');
