...
// ============================================
export function insertSatelliteData(payload) {
    const result = db.run(`INSERT INTO satellite_data (
...
// ============================================
export function getSatelliteDataByField(fieldId, limit = 100) {
...
// ============================================
export function insertWeatherDataWithForecast(payload) {
    return db.transaction(() => {
        // Insert main weather record
        const weatherResult = db.run(`INSERT INTO weather_data (
        field_id, timestamp, source, latitude, longitude,
      ...
      `, [
        ...
        if (payload.forecast && payload.forecast.length > 0) {
            const insertForecast = db.getConnection().prepare(`INSERT INTO weather_forecast (
...
// ============================================
export function getWeatherDataByField(fieldId, limit = 100) {
    const weatherData = db.query(`SELECT 
      id, field_id, timestamp, source,
      latitude, longitude,
    ...
    return weatherData.map((weather) => {
        const forecasts = db.query(`SELECT timestamp, temperature, humidity,
       precipitation_probability, precipitation
       FROM weather_forecast
       WHERE weather_data_id = ?
       ORDER BY timestamp ASC`, [weather.id]);
...
// ============================================
export function getLatestNDVI(fieldId) {
    return db.queryOne(`SELECT ndvi_mean, ndvi_min, ndvi_max, timestamp
     FROM satellite_data
     WHERE field_id = ?
     ORDER BY timestamp DESC
     LIMIT 1`, [fieldId]);
...
// ============================================
export function getNDVITrend(fieldId, days = 30) {
    return db.query(`SELECT 
    ...
    ORDER BY date ASC`, [fieldId, days]);
}
export default db;
