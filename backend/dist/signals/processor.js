 ...
 */
export function processSatelliteSignals(fieldId) {
    // Get all unprocessed satellite records
    const records = db.prepare('SELECT id, payload FROM satellite_records WHERE id NOT IN (SELECT source_id FROM signals WHERE source_type = ?)').all('satellite');
    const signals = [];
 ...
 */
export function processWeatherSignals(fieldId) {
    const records = db.prepare('SELECT id, payload FROM weather_records WHERE id NOT IN (SELECT source_id FROM signals WHERE source_type = ?)').all('weather');
    const signals = [];
    for (const record of records) {
        const data = JSON.parse(record.payload);
        if (data.fieldId === fieldId || !fieldId) {
 ...
 */
export function storeSignals(signals) {
  ...
  `);
    for (const signal of signals) {
        stmt.run(signal.source_type, signal.source_id, signal.field_id, signal.signal_type, signal.timestamp, signal.value, signal.metadata);
    }
 ...
 */
export function getSignals(fieldId, signalType) {
